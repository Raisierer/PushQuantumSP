...
class SPEvaluation:
    ...
    def check_solution(self):
        error = {
            "missing_achievable_coverage": [
                1 for i in range(self.missing_achievable_coverage)
            ]
        }
        return error
    ...
    def create_optimized_connections(self):
        ...
        for s in self.data.listStreetPoints3D:
            ...
            for l in self.listLidarActivated:
                ...
                if self.data._in_range(
                    l,
                    s,
                    self.data.rad_max,
                    self.data.vert_ang_max_deg,
                    self.data.vert_ang_min_deg,
        ...
        self.missing_achievable_coverage = (
            len(self.data.listStreetPoints3D)
            - len(self.listStreetPointsCovered)
            - len(self.data.listStreetPointsNeverCovered)
        )
