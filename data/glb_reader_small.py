...
def create_object(data, ax, mesh, xmintotal, xmaxtotal, ymintotal, ymaxtotal, walldir):
    ...
    if mesh["name"] == "WallMesh":
        vertices = np.array(
            [
                walldir % 2 * (x + size_x / 2, y - size_y / 2, z + size_z)
                + (1 - walldir % 2) * (x - size_x / 2, y + size_y / 2, z + size_z),
                (1 - walldir % 2) * (x + size_x / 2, y - size_y / 2, z + size_z)
                + walldir % 2 * (x - size_x / 2, y + size_y / 2, z + size_z),
            ]
        )
    ...
    if mesh["name"] == "WallMesh":
        ...
        basevertslist.append(
            [[baseverts[0][0], baseverts[0][1]], [baseverts[1][0], baseverts[1][1]]]
        )
    else:
        ...
        basevertslist.append(
            [[baseverts[1][0], baseverts[1][1]], [baseverts[2][0], baseverts[2][1]]]
        )
        basevertslist.append(
            [[baseverts[2][0], baseverts[2][1]], [baseverts[3][0], baseverts[3][1]]]
        )
        basevertslist.append(
            [[baseverts[3][0], baseverts[3][1]], [baseverts[0][0], baseverts[0][1]]]
        )
    ...
    ax.add_collection3d(
        Poly3DCollection([baseverts], linewidths=1, edgecolors="red", alpha=0.25)
    )
...
def create_street_points(xmin, xmax, ymin, ymax, xmargin, ymargin, density):
    res = []
    yloop = np.linspace(
        ymin + ymargin, ymax - ymargin, math.ceil((ymax - ymin) * density)
    )
    x = []
    for item in yloop:
        y = (
            np.linspace(
                xmin + xmargin, xmax - xmargin, math.ceil((xmax - xmin) * density)
            )
            * 0
            + item
        )
...
def create_problem_from_glb(
    lidar_density=0.1, street_point_density=0.1, save_as_json=False, show_plot=False
):
    ...
    for wall in walls:
        wallcount += 1
        xmintotal, xmaxtotal, ymintotal, ymaxtotal, basevertslist, size_z = (
            create_object(
                data, ax, wall, xmintotal, xmaxtotal, ymintotal, ymaxtotal, wallcount
            )
        )
    ...
    for box in boxes:
        xmintotal, xmaxtotal, ymintotal, ymaxtotal, basevertslist, size_z = (
            create_object(data, ax, box, xmintotal, xmaxtotal, ymintotal, ymaxtotal, 0)
        )
        for wal in basevertslist:
            wal.extend(
                [
                    size_z,
                    (size_z > pfostenlimit) * lidar_density,
                    lidar_height,
                    lidar_lateral_offset * (2 * (size_z <= pfostenlimit) - 1),
                    lidar_direction_mode,
                    lidar_pitch,
                ]
            )
    ...
    if save_as_json:
        ...
        outfile = (
            ...
            + "_lid"
            + str(lidar_density).replace(".", "p")
            + "_sp"
            + str(street_point_density).replace(".", "p")
            + date_time
            + ".json"
        )
