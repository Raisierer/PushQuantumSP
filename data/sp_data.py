...
class SPData:
    ...
    @classmethod
    def gen_problem(
        cls, num_cols, version, rad_max=2.5, hor_basic_distance=1, vert_basic_dist=2
    ):
        data_params = {
            "vert_ang_max_deg": 30,
            "vert_ang_min_deg": -70,
        ...
        if version == 1:
            return cls._gen_problem(
                num_cols,
                [0],
                0,
                (num_cols - 1) * hor_basic_distance,
                2.5,
                0,
                -10,
                0,
    ...
    def create_problem_from_glb_file(cls, lidar_density, street_point_density):
        problem_dict = create_problem_from_glb(
            lidar_density=lidar_density, street_point_density=street_point_density
        )
    ...
    def _gen_problem(
        ...
        **data_params,
        ...
        problem_dict = cls.problem_generator(
            l_number_per_row,
            l_y,
            l_xmin,
            l_xmax,
    ...
    def problem_generator(
        ...
        for i in l_y:
            lid.extend(
                cls.create_horizontal_lidar_points(
                    l_number_per_row,
                    i,
                    l_xmin,
                    l_xmax,
    ...
    @classmethod
    def create_horizontal_lidar_points(
        cls, number, y, xmin, xmax, height, yaw_deg, pitch_deg
    ):
        res = []
        x = []
        ...
        for s in self.listStreetPoints3D:
            ...
            for l in self.listLidar3D:
                ...
                if self._in_range(
                    ...
                    s,
                    self.rad_max,
                    self.vert_ang_max_deg,
                    self.vert_ang_min_deg,
                    ...
                    if inters == 0:
                        self.G.add_edge(
                            (s[0], s[1], s[2]), (l[0], l[1], l[2], l[3], l[4])
                        )
                        edgecount += 1
                        nc = 0
            if nc:
    ...
    def __generateGraph3D(self):
        ...
        for w in self.schemeGraph["wall"]:
            ...
            if w[3] > 0:
                ...
                difflidar = [
                    diff[0] * (1 - 2 * self.lidarwall_offset_m / length),
                ...
                difflidarstart = [
                    w[0][0]
                    + self.lidarwall_offset_m / length * diff[0]
                    + perpendicular_offset[0],
                    w[0][1]
                    + self.lidarwall_offset_m / length * diff[1]
                ...
                if numlid > 1:
                    for i in range(0, numlid):
                        ...
                        ang = (
                            90
                            - math.atan2(
                                perpendicular_offset[1], perpendicular_offset[0]
                            )
                            * 180
                            / math.pi
                        )
                        if w[6] > 0:
                            ...
                            ang += (
                                ((i + w[6]) % 2 - 0.5)
                                * 2
                                * (90 - self.halber_oeffnungswinkel_deg)
                            )
                    ...
                    ang = (
                        90
                        - math.atan2(perpendicular_offset[1], perpendicular_offset[0])
                        * 180
                        / math.pi
                    )
                    if w[6] > 0:
    ...
    def _in_range(
        ...
        vert_angle = math.atan2(
            rel_rotated[2], math.sqrt(rel_rotated[0] ** 2 + rel_rotated[1] ** 2)
        )
        if (
            vert_angle > vert_ang_max_deg / 180 * math.pi
            or vert_angle < vert_ang_min_deg / 180 * math.pi
        ):
            return 0
