...
class SPData: 
    ...
    def create_connections(self): 
        edgecount=0
        for idx, s in enumerate(self.listStreetPoints3D):
            nc=1
            for idx_lid, l in enumerate(self.listLidar3D):
                line=(l,s)
                if (self._in_range(l,s, self.rad_max, self.vert_ang_max_deg, self.vert_ang_min_deg, self.halber_oeffnungswinkel_deg)):
                    ...
                    if inters==0: 
                        ...
                        self.G.add_edge(f"sp{idx}", f"ld{idx_lid}")
                        edgecount+=1
                        nc=0
            if nc:
    ...
    def __generateGraph3D(self):
        ...
        for ind, elem in enumerate(pointsL3D):
            print(f"Adding node ld{ind} with pos={elem}")
            self.G.add_node(f"ld{ind}", pos=elem, type="lidar")

        for ind, elem in enumerate(pointsS3D):
            print(f"Adding node sp{ind} with pos={elem}")
            self.G.add_node(f"sp{ind}", pos=elem, type="streetpoint")

        #self.G.add_nodes_from(pointsL3D, type="lidar")
        #self.G.add_nodes_from(pointsS3D, type="streetpoint")

        print(f"Resulting graph: {self.G}")
    ...
    def __generateWalls(self):
        ...
        for idx, wall in enumerate(walls):
            self.M.add_node(f"w{idx}", pos=wall, type="wall")
        #self.M.add_nodes_from(walls, type="wall")
        return walls3D
