...
class SPPlot:
    ...
    def plot_solution(self, hide_never_covered=True):
        ...
        if hide_never_covered:
            street_points_covered = set(self.data.listStreetPoints3D) - set(
                self.data.listStreetPointsNeverCovered
            )
        ...
        nx.draw_networkx_nodes(
            self.evaluation.O,
            pos_opt,
            self.evaluation.listStreetPointsCovered,
            node_color="green",
            node_size=40,
        )
        nx.draw_networkx_nodes(
            self.data.G, pos, self.data.listLidar3D, node_color="orange", node_size=40
        )
        nx.draw_networkx_nodes(
            self.evaluation.O,
            pos_opt,
            self.evaluation.listLidarActivated,
            node_color="red",
            node_size=40,
        )
        ...
        nx.draw_networkx_edges(
            self.data.M, posw, edw, width=5.0, alpha=1, edge_color="black"
        )
    ...
    def plot_problem(self, draw_connections=True, hide_never_covered=True, show=False):
        ...
        if draw_connections:
            posl = {
                (node[0], node[1]): (node[0], node[1]) for node in self.data.G.nodes()
            }
            edl = {
                ((node[0][0], node[0][1]), (node[1][0], node[1][1])): (node[0], node[1])
                for node in self.data.G.edges()
            }
            nx.draw_networkx_edges(
                self.data.G, posl, edl, width=2.0, alpha=0.5, edge_color="green"
            )
